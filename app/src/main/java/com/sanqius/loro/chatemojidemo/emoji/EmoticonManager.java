...
/**
 * Created by loro on 2017/11/29 0029.
 * description ：
 */
...
public class EmoticonManager {
    ...
    private final List<Emotion> mDefaultEntries = new ArrayList<>();
    private final Map<String, Emotion> mText2Entry = new HashMap<>();
    private LruCache<String, Bitmap> mDrawableCache;
    private LruCache<String, AnimatedGifDrawable> mGifDrawableCache;
    private EmoManager mEmoManager;
    ...
    private EmoticonManager() {
        mEmoManager = EmoManager.getInstance();
        loadEmoticons();
    }
    ...
    private void loadEmoticons() {
        List<Emotion> mEmotionList = EmojiApplication.getEmotionList().mEmotionList;
        for (int i = 0; i < mEmotionList.size(); i++) {
            mText2Entry.put(mEmotionList.get(i).key, mEmotionList.get(i));
            mDefaultEntries.add(mEmotionList.get(i));
        }

//        load(mEmoManager.getContext(), mEmoManager.getEmotDir() + File.separator + mEmoManager.getConfigFile());
        //创建缓存
...
//        //补充最后一页少的表情,空白占位
//        int tmp = mDefaultEntries.size() % mEmoManager.getEmojiPerPage();
//        if (tmp != 0) {
//            int tmp2 = mEmoManager.getEmojiPerPage() - (mDefaultEntries.size() - (mDefaultEntries.size()
//                    / mEmoManager.getEmojiPerPage()) * mEmoManager.getEmojiPerPage());
...
//    }
    ...
    public String getDisplayText(int index) {
        return index >= 0 && index < mDefaultEntries.size() ? mDefaultEntries.get(index).key : null;
    }

    public Drawable getDisplayDrawable(Context context, int index) {
        String text = (index >= 0 && index < mDefaultEntries.size() ? mDefaultEntries.get(index).key : null);
    ...
    public Drawable getDrawable(Context context, String text) {
        Emotion entry = mText2Entry.get(text);
        if (entry == null || TextUtils.isEmpty(entry.key)) {
            return null;
        }
        Bitmap cache = mDrawableCache.get(entry.GetFilePath());
        if (cache == null) {
            cache = loadAssetBitmap(context, entry.GetFilePath());
        }
...
//        int size = IMApp.getInstance().getResources().getDimensionPixelSize(R.dimen.gif_width);
        int size = KeyboardUtils.dip2px(context, EmoManager.getInstance().getDefaultEmoBoundsDp());
//        int size = IMApp.getInstance().getResources().getDimensionPixelSize(R.dimen.gif_width);
        Emotion entry = mText2Entry.get(text);
        if (entry == null || TextUtils.isEmpty(entry.key)) {
            return null;
        }
        AnimatedGifDrawable cache = mGifDrawableCache.get(entry.GetFileGif());
        if (cache == null) {
            cache = loadAssetGif(context, entry.GetFileGif(), size);
            mGifDrawableCache.put(entry.GetFileGif(), cache);
        }
        return cache;
    }
    ...
    private boolean isFileExists(String filename) {
        AssetManager assetManager = mEmoManager.getContext().getAssets();
        try {
            String[] names = assetManager.list(mEmoManager.getEmotDir() + File.separator + mEmoManager.getSourceDir());
            for (String name : names) {
                String sourceName = mEmoManager.getEmotDir() + File.separator + mEmoManager.getSourceDir() + File.separator + name;
